...
COMANDO_MAX_CHARS = 300      # Comando maior que isso pode estar errado
...
def verificar_comando_vazio(questao: dict) -> str | None:
    ...
    if not questao.get("comando", "").strip():
        return "COMANDO_VAZIO: Separação comando/enunciado pode ter falhado"
    return None
...
def verificar_caracteres_estranhos(questao: dict) -> str | None:
    ...
    padroes_estranhos = [
        r'[�]',           # Caractere de substituição Unicode
        r'\\x[0-9a-f]{2}', # Escape hexadecimal
        r'[\x00-\x08]',   # Caracteres de controle
    ]
    ...
    for campo in campos:
        ...
        for padrao in padroes_estranhos:
            if re.search(padrao, texto):
                return f"ENCODING: Caracteres estranhos no campo '{campo}'"
...
def gerar_relatorio_texto(resultado: dict) -> str:
    ...
    if resultado["problematicas"]:
        ...
        for item in resultado["problematicas"]:
            ...
            linhas.append(f"  Comando: {questao.get('comando', '')[:100]}...")
...
def main():
    ...
    print("=" * 60)
    print("REVISOR DE QUESTÕES EXTRAÍDAS")
    print("=" * 60)
